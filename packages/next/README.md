...
"use server";
// ^ Required — this makes each exported function a real Next.js server action.
...
// The generated AgentProvider (components/betteragent-provider.tsx)
// imports this file automatically via `import *`.
...
**Do not import this file in your layout.** The `AgentProvider` generated by
`betteragent init` handles that with `import * as serverActions` — which is the
only way to consume a `"use server"` file in a Client Component without hitting
Next.js serialization boundary errors.
