...
✓ Signed in to acme-app (FREE).
  ...
  {
    name: "init",
    desc: "First-time setup wizard — installs a chat component, scaffolds tool files, generates AgentProvider, writes .env.local.",
            ...
            <div className="p-[14px_16px] border border-border rounded-[var(--radius-md)]">
              ...
              <div className="font-sans text-[13px] text-muted-foreground leading-[1.55]">
                <ol className="m-0 pl-4 flex flex-col gap-1">
                  ...
                  <li>Production default (<CodeChip>https://www.betteragent.dev</CodeChip>)</li>
                </ol>
              </div>
            </div>
          </div>
    ...
    <section className={SEC}>
      <div className={WRAP}>
        <div className={SECHEAD}>
          ...
          <p className={SUB}>Generated by <CodeChip>betteragent discover</CodeChip> or written by hand. <CodeChip>betteragent sync</CodeChip> reads and pushes them to the backend.</p>
        </div>
...
export const routes = [listProjects];`,
            },
            {
              ...
              desc: "Next.js Server Actions the agent invokes via the generated AgentProvider. Must start with \"use server\" and export each action individually — no array. Handlers must be imported from your own \"use server\" files.",
              code: `"use server";
...
// Export each action individually — no array.
// The generated AgentProvider picks them up automatically.
